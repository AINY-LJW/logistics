...
public class TSPData {
	 ...
	 */
	public float[][] disMap;
	public TSPData(List<Map<String, Double>> coors) {
		//路径集合
		CITY_NUM = coors.size();
		disMap = new float[CITY_NUM][CITY_NUM];
		for (int i = 0; i < CITY_NUM; i++) {
			for (int j = i; j < CITY_NUM; j++) {
				float dis = (float) Math.sqrt(Math.pow((coors.get(i).get("lat") - coors.get(j).get("lat")), 2)
						+ Math.pow((coors.get(i).get("lng") - coors.get(j).get("lng")), 2));
